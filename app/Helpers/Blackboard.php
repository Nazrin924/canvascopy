...
REQUEST_BODY;
    ...
    public static function find_name($user, $netID)
    {
        ...
        $results = $blackboard->User('getUser', ['filterType' => '6',
            'name' => $user], $netID);
        if (isset($results['id'])) {
    ...
    public static function checkCourse($id, $netID)
    {
        ...
        $results = $blackboard->Course('getCourse', ['filterType' => '2',
    ...
    public static function saveCourse($id, $name, $netID)
    {
        ...
        $results = $blackboard->Course('saveCourse', ['courseId' => $id,
        ...
        $resulting = $blackboard->Course('getCourse', ['filterType' => '2',
            'batchUids' => strtolower($id)], $netID);
        ...
        $user = $blackboard->User('getUser', ['filterType' => '6',
            'name' => $netID], $netID);
        sleep(5);
    ...
    public static function deleteCourse($id, $netID)
    {
        ...
        Blackboard::login($blackboard->getSessionId(), $netID);
        // Calling methods with complex types.
        $results = $blackboard->Course('deleteCourse', ['ids' => $id]);
    ...
    public static function deleteUser($userid, $netID)
    {
        ...
        Blackboard::login($blackboard->getSessionId(), $netID);
        // Calling methods with complex types.
        $results = $blackboard->User('deleteUser', ['userId' => $userid]);
        if (count($results) > 0) {
            return true;
        } else {
            \Log::info($results);
        ...
        $email, $account, $netID)
    {
        ...
        \Log::info("Datasource key is $dsk");
        // Calling methods with complex types.
        $results = $blackboard->User('saveUser', ['name' => $account,
            'dataSourceId' => $dsk,
            'isAvailable' => 'true',
            'extendedInfo' => [
                'emailAddress' => $email,
    ...
    public static function change_user_batch_uid($old, $new, $sessionid, $netID)
    {
        ...
        if ($debug) {
            echo "changeUserBatchUid routine:\n";
        }
...
REQUEST_BODY;
        ...
        if ($return) {
            $id = (string) $return[0];
            if ($debug) {
                echo "changeUserBatchUid returning return value: $id\n";
            }

            return $id;
        } else {
            if ($debug) {
                echo "changeUserBatchUid returning Did not update user\n";
            }
        ...
        $sessionid, $netID)
    {
        ...
        if ($debug) {
            echo "changeUserDataSourceId routine:\n";
        }
...
REQUEST_BODY;
        ...
        if ($return) {
            $id = (string) $return[0];
            if ($debug) {
                echo "changeUserDataSourceId returning return value: $id\n";
            }

            return $id;
        } else {
            if ($debug) {
                echo "changeUserDataSourceId returning Did not update user\n";
            }
